    ...
    <div className="flex justify-center items-center gap-4 py-6">
      {values.map((digit, index) => (
        <input
          ...
          className={`w-16 h-16 bg-[#0A0A0A] border-2 rounded-2xl text-3xl font-bold text-center text-white focus:outline-none transition-all ${
            digit
              ? 'border-[#FF6B6B]/50'
              : 'border-[#222222]'
          } focus:border-[#FF6B6B] focus:ring-1 focus:ring-[#FF6B6B] focus:shadow-sm focus:shadow-[#FF6B6B]/20`}
        />
      ))}
    </div>
  );
    ...
    return (
      <div className="flex flex-col min-h-full px-6 py-8 bg-[#050505]">
        <div className="flex-1 flex flex-col items-center justify-center text-center">
          ...
          <h2 className="text-2xl font-bold text-[#F1F1F1] mb-3">Kill Switch Set!</h2>
          ...
          <div className="mt-6 flex justify-center gap-3">
            {code.map((digit, i) => (
              <span key={i} className="w-12 h-14 flex items-center justify-center text-2xl font-bold bg-[#0A0A0A] border-2 border-[#FF6B6B] text-white rounded-2xl">
                {digit}
              </span>
            ))}
          </div>
          <p className="mt-4 text-sm text-[#636363]">Remember this code!</p>
        </div>
  ...
  return (
    <div className="flex flex-col min-h-full px-6 py-8 bg-[#050505]">
      <div className="flex-1">
        ...
        {error && (
          <div className="bg-[#EF4444]/10 border border-[#EF4444]/30 rounded-2xl p-3 mb-6">
        ...
        {step === 'enter' && (
          <div className="bg-[#0D0D0D] rounded-2xl p-4 border border-[#1A1A1A]">
            <h4 className="font-medium text-[#E8ECF1] mb-2">When to use the kill switch:</h4>
            <ul className="space-y-1 text-sm text-[#B0B0B0]">
              <li className="flex items-center gap-2">
                <span className="w-1.5 h-1.5 bg-[#636363] rounded-full" />
        ...
        {step === 'confirm' && (
          <button
            onClick={handleReset}
            className="w-full py-2 text-[#FF8A8A] font-medium text-sm hover:text-[#FF6B6B] transition-colors"
          >
