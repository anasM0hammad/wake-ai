...
export default function BatteryOptimization({ onNext, onBack }) {
  ...
  if (isChecking) {
    return (
      <div className="flex flex-col items-center justify-center min-h-full px-6 py-8 bg-[#0a0a0a]">
        <div className="w-12 h-12 border-4 border-[#333333] border-t-indigo-500 rounded-full animate-spin mb-4" />
        <p className="text-neutral-400">Checking battery settings...</p>
      </div>
    );
  }
    ...
    <div className="flex flex-col min-h-full px-6 py-8 bg-[#0a0a0a]">
      <div className="flex-1">
        <div className="mb-6 text-center">
          <div className="w-20 h-20 mx-auto mb-6 rounded-full bg-amber-900/50 flex items-center justify-center">
        ...
        <div className="bg-amber-900/30 border border-amber-700/50 rounded-xl p-4 mb-6">
          <div className="flex gap-3">
            ...
            <div>
              <h4 className="font-semibold text-amber-300 mb-1">Why disable this?</h4>
        ...
        {showInstructions && instructions && (
          <div className="bg-[#171717] rounded-xl p-4 mb-6 border border-[#262626]">
            ...
            <ol className="space-y-2">
              {instructions.steps.map((step, index) => (
                <li key={index} className="flex gap-3 text-sm text-neutral-300">
                  <span className="w-6 h-6 rounded-full bg-indigo-900/50 text-indigo-400 flex items-center justify-center flex-shrink-0 font-medium text-xs">
                    {index + 1}
                  </span>
        ...
        {!showInstructions && (
          <button
            ...
            className="w-full py-3 px-4 bg-indigo-900/50 text-indigo-300 font-medium rounded-xl hover:bg-indigo-900/70 transition-colors flex items-center justify-center gap-2"
          >
      ...
      <div className="space-y-3 pt-6">
        ...
        {!showInstructions && (
          <button
            onClick={onNext}
            className="w-full py-3 px-6 text-neutral-500 font-medium hover:text-neutral-300 transition-colors text-sm"
          >
