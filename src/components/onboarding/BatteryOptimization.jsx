...
export default function BatteryOptimization({ onNext, onBack }) {
  ...
  if (isChecking) {
    return (
      <div className="flex flex-col items-center justify-center min-h-full px-6 py-8 bg-[#09090B]">
        <div className="w-12 h-12 border-4 border-zinc-700 border-t-sky-500 rounded-full animate-spin mb-4" />
        <p className="text-zinc-400">Checking battery settings...</p>
      </div>
    );
  }
    ...
    <div className="flex flex-col min-h-full px-6 py-8 bg-[#09090B]">
      <div className="flex-1">
            ...
            <ol className="space-y-2">
              {instructions.steps.map((step, index) => (
                <li key={index} className="flex gap-3 text-sm text-zinc-300">
                  <span className="w-6 h-6 rounded-full bg-sky-900/50 text-sky-400 flex items-center justify-center flex-shrink-0 font-medium text-xs">
                    {index + 1}
                  </span>
        ...
        {!showInstructions && (
          <button
            ...
            className="w-full py-3 px-4 bg-sky-900/30 text-sky-300 font-medium rounded-xl hover:bg-sky-900/50 transition-colors flex items-center justify-center gap-2"
          >
      ...
      <div className="space-y-3 pt-6">
        ...
        {!showInstructions && (
          <button
            onClick={onNext}
            className="w-full py-3 px-6 text-zinc-500 font-medium hover:text-zinc-300 transition-colors text-sm"
          >
