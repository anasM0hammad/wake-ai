  ...
  if (isChecking) {
    return (
      <div className="flex flex-col items-center justify-center min-h-full px-6 py-8 bg-[#050505]">
        <div className="w-12 h-12 border-4 border-[#222222] border-t-[#FF6B6B] rounded-full animate-spin mb-4" />
        <p className="text-[#636363]">Checking battery settings...</p>
      </div>
    );
  }
    ...
    <div className="flex flex-col min-h-full px-6 py-8 bg-[#050505]">
      <div className="flex-1">
        <div className="mb-6 text-center">
          <div className="w-20 h-20 mx-auto mb-6 rounded-full bg-[#FF6B6B]/10 border border-[#FF6B6B]/20 flex items-center justify-center">
            <svg className="w-10 h-10 text-[#FF6B6B]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 10V3L4 14h7v7l9-11h-7z" />
            </svg>
          </div>
          <h2 className="text-2xl font-bold text-[#F1F1F1] mb-3">Battery Optimization</h2>
          <p className="text-[#B0B0B0] max-w-sm mx-auto">
        ...
        <div className="bg-[#FF6B6B]/10 border border-[#FF6B6B]/20 rounded-2xl p-4 mb-6">
          <div className="flex gap-3">
            ...
            <div>
              <h4 className="font-semibold text-[#F1F1F1] mb-1">Why disable this?</h4>
        ...
        {showInstructions && instructions && (
          <div className="bg-[#0D0D0D] rounded-2xl p-4 mb-6 border border-[#1A1A1A]">
            <h4 className="font-semibold text-[#E8ECF1] mb-3">{instructions.title}</h4>
            <ol className="space-y-2">
              {instructions.steps.map((step, index) => (
                <li key={index} className="flex gap-3 text-sm text-[#B0B0B0]">
                  <span className="w-6 h-6 rounded-full bg-[#FF6B6B]/20 text-[#FF6B6B] flex items-center justify-center flex-shrink-0 font-medium text-xs">
                    {index + 1}
                  </span>
        ...
        {!showInstructions && (
          <button
            ...
            className="w-full py-3 px-4 bg-[#FF6B6B]/10 border border-[#FF6B6B]/20 text-[#FF8A8A] font-medium rounded-2xl hover:bg-[#FF6B6B]/20 transition-colors flex items-center justify-center gap-2"
          >
      ...
      <div className="space-y-3 pt-6">
        ...
        {!showInstructions && (
          <button
            onClick={onNext}
            className="w-full py-3 px-6 text-[#636363] font-medium hover:text-[#FF8A8A] transition-colors text-sm"
          >
